...
import { Users, Lightbulb, CheckCircle2, ShieldCheck, AlertCircle, LayoutDashboard, LogOut, Search, Check, XCircle, ChevronRight, DollarSign } from "lucide-react";
        ...
        <div className="max-w-[1600px] mx-auto px-8 py-5 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <div className="relative">
              ...
              <Input placeholder="Search global data..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="bg-white/5 border-white/10 pl-10 h-10 w-64 rounded-xl text-xs focus:ring-indigo-600 transition-all" />
            </div>
            <Button variant="ghost" onClick={() => navigate("/")} className="text-slate-400 hover:text-white hover:bg-white/5 rounded-xl font-bold text-xs"><LogOut className="w-4 h-4 mr-2" /> Exit</Button>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-[1600px] mx-auto px-8 py-10">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-10">
          {[
            ...
            { label: "Live Ventures", value: ideas.length, icon: Lightbulb, color: "emerald" },
        ...
        <Tabs defaultValue="users" className="space-y-6">
          <TabsList className="bg-white/5 border border-white/10 p-1 rounded-xl">
            ...
            <TabsTrigger value="ideas" className="rounded-lg px-6 font-bold py-2 data-[state=active]:bg-indigo-600 data-[state=active]:text-white transition-all">Idea Pipeline</TabsTrigger>
          </TabsList>
              ...
              {filteredUsers.map((user) => (
                <div key={user.id} className="bg-[#161922] border border-white/5 p-4 rounded-2xl flex items-center justify-between hover:border-white/10 transition-all group">
                  ...
                  <div className="flex items-center gap-6">
                    ...
                    <Button variant={user.is_approved ? "outline" : "default"} onClick={() => toggleApprove(user.id, !!user.is_approved)} className={`rounded-xl h-10 px-6 font-bold text-xs shadow-lg transition-all ${!user.is_approved ? 'bg-indigo-600 hover:bg-indigo-700 text-white' : 'border-white/10 text-slate-400 hover:text-white hover:border-white/20'}`}>
                ...
                <div key={idea.id} className="bg-[#161922] border border-white/5 p-5 rounded-2xl flex items-center justify-between hover:border-white/10 transition-all group">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      {idea.status === 'deal_done' && <Badge className="bg-emerald-500/10 text-emerald-500 border-0 text-[10px] font-black tracking-widest uppercase flex gap-1"><CheckCircle2 size={12}/> Closed</Badge>}
                    </div>
                    <div className="flex items-center gap-4 text-xs text-slate-500 font-medium">
                      <span className="flex items-center gap-1.5"><Users size={14}/> {idea.founder?.name}</span>
                      <span className="flex items-center gap-1.5"><DollarSign size={14}/> Target: ${idea.investment_needed?.toLocaleString()}</span>
                      <span className="flex items-center gap-1.5"><ChevronRight size={14}/> Current: {idea.status || 'Pending'}</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    <select onChange={(e) => updateIdeaStatus(idea.id, e.target.value)} value={idea.status || ""} className="bg-[#0f1115] border border-white/10 rounded-xl px-4 h-10 text-xs font-bold text-slate-400 focus:outline-none hover:border-white/20 transition-all">
