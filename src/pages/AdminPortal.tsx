...
const AdminPortal = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const { data: investData } = await supabase.from("investment_records").select("*, founder:profiles!founder_id(name), investor:profiles!investor_id(name), idea:ideas(*)").order("created_at", { ascending: false });
  ...
  return (
    <div className="min-h-screen bg-[#0f172a] text-slate-200 font-sans antialiased overflow-hidden flex relative">
      ...
      <TooltipProvider>
        ...
        {/* Sidebar */}
        <aside
          className={`bg-[#020617] border-r border-slate-800 transition-all duration-300 flex flex-col z-50 ${isSidebarOpen ? 'w-64' : 'w-20'}`}
        >
            ...
            {navigation.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all group ${isActive ? 'bg-indigo-600/10 text-indigo-400' : 'text-slate-400 hover:bg-slate-800 hover:text-slate-200'}`}
                >
                  <Icon size={20} className={isActive ? 'text-indigo-400' : 'group-hover:text-slate-200'} />
                  {isSidebarOpen && <span className="font-semibold text-sm">{item.label}</span>}
                  {isActive && isSidebarOpen && <div className="ml-auto w-1.5 h-1.5 rounded-full bg-indigo-500 shadow-[0_0_10px_rgba(99,102,241,0.5)]" />}
                </button>
              );
            })}
          </nav>
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          {/* Header */}
          <header className="h-20 bg-[#0f172a]/80 backdrop-blur-md border-b border-slate-800 flex items-center justify-between px-8 z-40">
            ...
            <div className="flex items-center gap-6">
              <div className="relative group hidden md:block">
                ...
                <Input
                  ...
                  className="bg-slate-900 border-slate-800 pl-10 h-10 w-64 rounded-xl text-xs focus:ring-1 focus:ring-indigo-500 transition-all placeholder:text-slate-600 font-medium text-white"
                />
              </div>
              ...
              <div className="flex items-center gap-4">
                <div className="flex -space-x-2">
                  ...
                  <div className="w-8 h-8 rounded-full bg-indigo-600/20 text-indigo-400 border-2 border-[#0f172a] flex items-center justify-center text-[10px] font-bold">+24</div>
                </div>
            ...
            <AnimatePresence mode="wait">
              {activeTab === 'overview' && (
                ...
                >
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                    {[
                      { label: "Platform Revenue", value: `₹${payments.reduce((acc, p) => acc + (Number(p.amount) || 0), 0).toLocaleString()}`, icon: DollarSign, trend: "+12.5%", color: "indigo" },
                    ...
                    <Card className="col-span-12 lg:col-span-8 bg-slate-900/50 border-slate-800 border shadow-sm rounded-2xl overflow-hidden">
                      <CardHeader className="p-6 border-b border-slate-800/50 flex flex-row items-center justify-between">
                        ...
                        <div className="flex gap-1 bg-slate-800/50 p-1 rounded-lg">
                          {['7D', '1M', '1Y'].map(t => (
                            <button key={t} className={`px-3 py-1 text-[10px] font-bold rounded-md transition-all ${t === '1M' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-500 hover:text-slate-300'}`}>{t}</button>
                          ))}
                        </div>
                      </CardHeader>
                        ...
                        <tbody className="divide-y divide-slate-800">
                          {filteredUsers.map((user) => (
                            <tr key={user.id} className="hover:bg-slate-900/30 group transition-colors cursor-pointer" onClick={() => setSelectedUser(user)}>
                              ...
                              <td className="py-4 px-6">
                                <Badge className={`${user.user_type === 'investor' ? 'bg-indigo-500/10 text-indigo-400 border-indigo-500/20' : 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20'} border text-[9px] font-black uppercase tracking-tighter px-2`}>
                                  {user.user_type}
                                </Badge>
                              </td>
                ...
                <motion.div initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {filteredIdeas.map((idea) => (
                      <Card key={idea.id} className="bg-slate-900 border-slate-800 shadow-xl rounded-2xl overflow-hidden hover:border-indigo-500/30 transition-all cursor-pointer group" onClick={() => setSelectedIdea(idea)}>
                        <div className="p-6 space-y-4">
                          ...
                          <h3 className="text-lg font-bold text-white leading-tight group-hover:text-indigo-400 transition-colors uppercase tracking-tight">{idea.title}</h3>
                          ...
                          <div className="pt-4 border-t border-slate-800 flex items-center justify-between">
                            <div className="flex items-center gap-2">
                              <Avatar className="w-6 h-6 border border-slate-800">
                                <AvatarFallback className="text-[8px] bg-indigo-500/10 text-indigo-400 font-bold">{idea.founder?.name?.charAt(0)}</AvatarFallback>
                              </Avatar>
                            ...
                            <div className="text-right">
                              ...
                              <p className="text-xs font-black text-indigo-400">₹{idea.investment_needed?.toLocaleString()}</p>
                            </div>
                          </div>
                        </div>
                      </Card>
              ...
              {activeTab === 'tools' && (
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  {[
                    { title: "Notification Engine", desc: "Push global infrastructure updates", icon: Bell, btn: "Dispatch Blast", color: "indigo" },
                    ...
                    { title: "Support Interface", desc: "High-priority ticket management", icon: Mail, btn: "Open Desk", color: "purple" }
            ...
            <ScrollArea className="max-h-[85vh]">
              <div className="p-10">
                <div className="flex items-center gap-6 mb-10">
                  <Avatar className="w-16 h-16 border-2 border-indigo-500/20">
                  ...
                  <div>
                    <div className="flex items-center gap-3">
                      ...
                      <Badge className="bg-indigo-500/20 text-indigo-400 border-none font-black text-[9px] uppercase tracking-widest">{selectedUser?.user_type}</Badge>
                    </div>
                ...
                {selectedUser?.user_type === 'investor' && (
                  <div className="mb-10 bg-indigo-600/5 border border-indigo-500/20 p-8 rounded-2xl">
                    <p className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.2em] mb-4">Investment Framework</p>
                    <div className="flex items-end justify-between gap-6">
                      <div>
                        <p className="text-[9px] text-indigo-300 font-bold uppercase mb-1">Stated Capital Facility</p>
                      ...
                      <div className="flex flex-wrap gap-2 justify-end">
                        {selectedUser?.interested_domains?.map((d: string) => (
                          <Badge key={d} className="bg-indigo-500/10 text-indigo-400 border-none font-black text-[9px] tracking-widest">{d}</Badge>
                        ))}
                      </div>
                    </div>
                  </div>
              ...
              <div className="p-12">
                <div className="flex items-start justify-between mb-12">
                  <div className="space-y-4">
                    <div className="flex gap-2">
                      <Badge className="bg-indigo-600/10 text-indigo-400 border-none font-black text-[9px] tracking-widest">{selectedIdea?.domain}</Badge>
                ...
                <div className="grid grid-cols-12 gap-12">
                  <div className="col-span-8 space-y-12">
                    <section>
                      <h5 className="text-[10px] font-black text-indigo-500 uppercase tracking-[0.3em] mb-4 flex items-center gap-2">
                    ...
                    <div className="grid grid-cols-3 gap-6">
                      {[
                        { label: "Market Size", val: selectedIdea?.market_size || "Lite", icon: Globe, color: "indigo" },
                        { label: "Traction", val: selectedIdea?.traction || "Early", icon: TrendingUp, color: "emerald" },
                        { label: "Core Team", val: selectedIdea?.team_size || "1", icon: Users, color: "purple" }
                      ...
                      <div className="bg-slate-900 border border-slate-800/50 p-8 rounded-[2rem] space-y-8">
                        ...
                        <div className="space-y-3">
                          <div className="flex justify-between text-[10px] font-black uppercase text-slate-600">
                            ...
                            <span className="text-indigo-500">{Math.round(((selectedIdea?.investment_received || 0) / (selectedIdea?.investment_needed || 1)) * 100)}%</span>
                          </div>
                          <div className="h-2 w-full bg-slate-800 rounded-full overflow-hidden">
                            <div className="h-full bg-indigo-500 rounded-full shadow-[0_0_10px_rgba(99,102,241,0.5)]" style={{ width: `${Math.min(((selectedIdea?.investment_received || 0) / (selectedIdea?.investment_needed || 1)) * 100, 100)}%` }} />
                          </div>
                        </div>
                      </div>
                    </section>
                    ...
                    <section>
                      ...
                      <div className="space-y-3">
                        {selectedIdea?.media_url && (
                          <a href={selectedIdea.media_url} target="_blank" className="flex items-center justify-between p-5 bg-slate-900 rounded-2xl border border-slate-800 hover:border-indigo-500 transition-all group">
                            <div className="flex items-center gap-4">
                              <div className="w-9 h-9 rounded-xl bg-indigo-500/10 text-indigo-400 flex items-center justify-center"><FileText size={18} /></div>
                ...
                <div className="flex items-center justify-between border-t border-slate-800 pt-10 sticky bottom-0 bg-[#0f172a] -mx-12 -mb-12 p-12 mt-12 z-20">
                  ...
                  <div className="flex gap-4">
                    ...
                    <Button className="bg-indigo-600 hover:bg-indigo-700 text-white font-black uppercase text-[10px] tracking-widest px-10 rounded-xl" onClick={() => setSelectedIdea(null)}>Commit Decisions</Button>
                  </div>
                </div>
              </div>
            </ScrollArea>
