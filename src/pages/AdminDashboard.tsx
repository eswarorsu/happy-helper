    ...
    <div className="min-h-screen bg-[#0f172a] text-slate-200 flex font-sans">
      <aside className="w-64 border-r border-slate-800 bg-[#1e293b]/50 backdrop-blur-xl flex flex-col fixed h-full z-20">
            ...
            {[
              ...
              <button
                ...
                className={`w-full flex items-center justify-between p-3 rounded-xl transition-all group ${
                  view === item.id ? "bg-indigo-600/10 text-indigo-400 border border-indigo-600/20" : "text-slate-400 hover:bg-slate-800/50 hover:text-slate-200"
                }`}
              >
        ...
        <div className="mt-auto p-8">
          <Button variant="ghost" className="w-full justify-start gap-3 text-slate-400 hover:text-red-400 hover:bg-red-400/10 rounded-xl" onClick={handleLogout}>
        ...
        <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in duration-700">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            {[
              ...
              <Card key={i} className="bg-slate-900/50 border-slate-800 shadow-2xl rounded-2xl">
                <CardContent className="p-6">
                  <div className="flex items-center justify-between mb-4">
                    <div className="p-2.5 rounded-xl bg-indigo-500/10 text-indigo-400"><stat.icon className="w-5 h-5" /></div>
                    {stat.alert && <Badge className="bg-red-500/20 text-red-400 border-red-500/20 animate-pulse">Action Required</Badge>}
                  </div>
          ...
          <Card className="bg-slate-900/50 border-slate-800 rounded-2xl overflow-hidden shadow-2xl">
            <CardHeader className="border-b border-slate-800 bg-slate-900/20 text-white">
            ...
            <CardContent className="p-0">
              <div className="divide-y divide-slate-800">
                {view !== "ideas" && users.filter(u => view === "users" ? true : !u.is_approved).slice(0, 10).map((user) => (
                  <div key={user.id} className="p-4 hover:bg-slate-800/30 flex items-center justify-between group">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-slate-500 font-bold uppercase">{user.user_type}</p>
                      </div>
                    </div>
                    <div className="flex gap-2">
                      <Button size="icon" variant="ghost" className={user.is_approved ? "text-green-400" : "text-slate-500"} onClick={() => verifyUser(user.id, !user.is_approved)}><CheckCircle2 className="w-4 h-4" /></Button>
                      <Button size="icon" variant="ghost" className="text-slate-500 hover:text-red-400"><XCircle className="w-4 h-4" /></Button>
                    </div>
                  </div>
                ))}
                {view !== "users" && ideas.filter(i => view === "ideas" ? true : i.status === "pending").slice(0, 10).map((idea) => (
                  <div key={idea.id} className="p-4 hover:bg-slate-800/30 flex items-center justify-between group border-l-4 border-l-indigo-600/30">
                    <div className="flex items-center gap-4">
                      <div className="p-2 rounded-lg bg-indigo-600/10 text-indigo-400"><Lightbulb className="w-5 h-5" /></div>
                      <div>
                        <p className="font-bold text-white text-sm">{idea.title}</p>
                        <p className="text-[10px] text-slate-500 font-bold uppercase">By {idea.founder?.name}</p>
                      </div>
                    </div>
                    <div className="flex gap-2">
